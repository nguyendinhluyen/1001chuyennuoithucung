...
function getNextRowLayout(oTable, oTR, oTD)
{
    ...
    for (var k = 0; k < rSpan; k++)
        oTR_tmp = oTR_tmp.nextSibling;
    if (!oTR_tmp)
        return null
    //alert(oTR_tmp.outerHTML)
...
function splitRow()
{
    ...
    if (getSpanNum(oTD.getAttribute("ROWSPAN")) == 1)
        return;//"TD tdk ada rowSpan
...
function splitCol()
{
    ...
    if (getSpanNum(oTD.getAttribute("COLSPAN")) == 1)
        return;
...
function rowOperation(bGetArrayCells, bDoInsertRowAbove, bDoInsertRowBelow, bDeleteRow)
{
    ...
    for (var i = 0; i < oTR.childNodes.length; i++)
    {
        ...
        var cSpan = getSpanNum(oTD_tmp.getAttribute("COLSPAN"));
        while (arrAllCols[nCount][0] != null)
            nCount++
